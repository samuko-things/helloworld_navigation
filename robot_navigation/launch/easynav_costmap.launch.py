...
def generate_launch_description():
    ...
    costmap_route_params_file = LaunchConfiguration('costmap_route_params_file')
    costmap_params_file = LaunchConfiguration('costmap_params_file')
    use_route = LaunchConfiguration('use_route')
    # rviz_config = LaunchConfiguration('rviz_config')


    declare_costmap_route_params_file_cmd = DeclareLaunchArgument(
                'costmap_route_params_file',
                default_value=os.path.join(
                    robot_nav_pkg_path, 'config', 'easynav_costmap_route_params.yaml'),
    ...
    declare_use_route_argument = DeclareLaunchArgument(
            'use_route',
            default_value='false',
            description='use easynav route with costmap if true')
    ...
    easynav_system_costmap_route = Node(
        package='easynav_system',
        executable='system_main',
        parameters=[costmap_route_params_file],
        output='screen',
        remappings=[
    ...
    ld.add_action(declare_costmap_route_params_file_cmd)
    ld.add_action(declare_costmap_params_file_cmd)
    ld.add_action(declare_use_route_argument)
    # ld.add_action(declare_rviz_config_cmd)

    ld.add_action(easynav_system_costmap_route)
